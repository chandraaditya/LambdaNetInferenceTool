...
object ProgramParsing {
  ...
  def parseGFile(
     tsFile: Path,
     parsingFromFilets: Path,
   ): GProject = {
    val subProjects = Map.empty[ProjectPath, RelPath]
    val baseDirs = Map.empty[Dir, Option[ProjectPath]]
    val aliases = Map.empty[ProjectPath, ProjectPath]
    val mapping = BasicPathMapping(subProjects, baseDirs, aliases)
    val devDependencies = Set.empty[ProjectPath]

    GProject(
      tsFile, // file name as project name
      ProgramParsing.parseGModulesFromFile(tsFile, parsingFromFilets),
      mapping,
      subProjects,
      devDependencies
    )
...
//    println(packageFiles)
//    println("done printing files")
    //    println(packageFiles.toString())
    val subProjects = Map.empty[ProjectPath, RelPath]
    //      (for {
//        (f, pkg) <- packageFiles
//        name <- pkg.moduleName
...
//    }
    ...
    val aliases: Map[ProjectPath, ProjectPath] = Map()
//      (for {
...
//    println(mapping)
    ...
    println("start printing root")
    println(root.toString())
    println("end printing root")
...
//    }
    ...
    val devDependencies: Set[ProjectPath] = Set()
...
//      .flatMap(_.devDependencies.flatMap(handleTypesPrefix))
    ...
    val a = ProgramParsing.parseGModulesFromFiles(sources, root)
...
//    println("here 1")
  ...
  def parseGModulesFromFile(
      tsFile: Path,
      parsingFromFilejs: Path
  ): Vector[GModule] =
    SimpleMath.withErrorMessage {
      val command = s"node ${parsingFromFilejs.toString()} " +
        s"--src ${tsFile.toString()}" +
        s"--lib ${tsFile.toString()}"
      s"Failed command: $command"
    } {
      val r = %%(
        'node,
        parsingFromFilejs.toString(),
        "--src",
        tsFile.toString(),
        "--lib",
        tsFile.toString()
      )(Path.home)
      var parsedJson = r.out.string

      parsedJson = parsedJson.replace(tsFile.toString(), tsFile.toString().split('/').takeRight(1).mkString(""))
//
...
//      println("done printing")
  ...
  def parseGModulesFromFiles(
    ...
    SimpleMath.withErrorMessage {
      ...
      println("printing parsed file")
      println(parsedJson)
      println("done printing parsed file")
      ProgramParsing().parseGModulesFromJson(parsedJson)
    }
