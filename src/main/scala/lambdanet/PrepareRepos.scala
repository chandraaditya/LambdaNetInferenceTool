...
//    val baseCtx1 = baseCtx |+| ModuleExports(publicSymbols = publicNamespaces)
  ...
  def parseProject(
    ...
    SimpleMath.withErrorMessage(s"In project: $projectRoot") {
      ...
      //TODO: replace this with custom class or function as it parses the files
...
//      println("project name: " + projectName)
  ...
  def parseFile(
    ...
    SimpleMath.withErrorMessage(s"In project: $tsFile") {
      import libDefs._

      val p = ProgramParsing.parseGFile(tsFile, parsingFromFilets)

      val allocator = new PNodeAllocator(forLib = false)
      val irTranslator = new IRTranslation(allocator)

      val projectName = tsFile.toString().split('/').takeRight(1).mkString("")
      val qModules = QLangTranslation.fromProject(
        projectName,
        p.modules,
        baseCtx,
      ...
      if (warnOnErrors)
        errorHandler.warnErrors()
//      printResult(s"File parsed: '$tsFile'")
//      println("number of nodes: " + graph.nodes.size)
