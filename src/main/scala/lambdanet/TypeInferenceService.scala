...
object TypeInferenceService {
  ...
  case class CustomPredictionResults(
    ...
    def prettyPrint(): Unit = {
      ...
      byFile.foreach {
        case (file, nodes) =>
          println(s"Predictions for file: $file")
          nodes.toSeq.sortBy(_.srcSpan.get.start).foreach { n =>
            val span = n.srcSpan.get.showShort()
            val lineNumber = n.srcSpan.get.until._1 + 1
            val column2 = n.srcSpan.get.until._2 + 1
            val column1 = n.srcSpan.get.start._2 + 1
            val rankedList = map(n).distr.zipWithIndex
              ...
              .mkString(", ")
            println(s"---------------------------------------------------")
            println(s"On line $lineNumber, for variable from column $column1 to $column2 add a suggested type\nSuggestions: $rankedList")
          }
      }
    }
  }
